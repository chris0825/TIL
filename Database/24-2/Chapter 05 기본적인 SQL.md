...
# DROP문
> 테이블, 도메인, 제약ㅈ건과 같은 이름을 가진 스키마 요소 삭제
## 제거 옵션
- CASECADE (연쇄)
  > 제거하려는 스키마에 포함된 모든 요소까지 삭제
- RESTRICT (제한)
  > 내용이 비어있는 것만 삭제
```sql
DROP SCHEMA또는TABLE 이름 제거옵션;
```
...
## 열(애트리뷰트) 변경 옵션
- CASCADE (연쇄)
  > 제거할 열과 그 열을 참조하는 모든 것(제약 조건, 뷰 등) 수정
- RESTRICT (제한)
  > 그 열을 참조하는 뷰나 제약 조건이 없는 경우만 수정
```sql
ALTER TABLE 테이블명 수행할연산 COLUMN또는CONSTRAINT 애트리뷰트명 변경옵션;
```
